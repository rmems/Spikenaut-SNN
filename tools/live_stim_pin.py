#!/usr/bin/env python3
"""The analog stim parity pin: build it, load it, compare it (issue #52).
...
``hamming_encode.encode_record`` is the reference that builds that ``stim``,
and ``spikenaut_snn::stim::LiveStimAdapter`` is the Rust implementation of the
same contract. This module holds the artifact the two are compared through.

The CLI is ``live_stim_parity``; the self-test is ``live_stim_selftest``. Same
split as ``hamming_core`` / ``hamming_selftest`` / ``measure_hamming`` and
``q88_core`` / ``q88_selftest`` / ``verify_q88`` next door.
