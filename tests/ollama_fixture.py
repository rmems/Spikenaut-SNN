...
def ollama_server(**options):
    ...
    Handler = _handler_type(state, config)
...
def _handler_type(state, config):
    class Handler(BaseHTTPRequestHandler):
        ...
        def _models_response(self):
            ...
            models = _resident_models(state, config)
        ...
        def _generate_response(self, payload):
            ...
            if config.preload_header_drip_interval and payload.get("keep_alive") != 0:
                return self._drip_json(
                    response, config.preload_header_drip_interval, include_headers=True
                )
            if config.preload_drip_interval and payload.get("keep_alive") != 0:
                return self._drip_json(response, config.preload_drip_interval)
            self._json(response)
        ...
        def _load_model(self, payload):
            if config.preload_never_completes_delay:
                time.sleep(config.preload_never_completes_delay)
                return False
            if config.preload_visibility_delay:
                time.sleep(config.preload_visibility_delay)
            state["loaded"] = True
            state["ever_loaded"] = True
            state["model"] = config.post_load_model or payload["model"]
            state["context"] = config.post_load_context or payload["options"]["num_ctx"]
            if config.preload_response_delay:
                time.sleep(config.preload_response_delay)
            return True
