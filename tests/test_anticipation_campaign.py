...
def test_cleanup_interrupt_persists_incomplete_capture_status(tmp_path):
    ...
    stimulus = InterruptingStimulus()

    with pytest.raises(KeyboardInterrupt):
        campaign._finish_capture(tmp_path, stimulus, status, True, False)
...
def test_capture_rejects_collector_changed_after_preflight(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="changed after campaign preflight"):
        campaign._capture_session(session, NoopStimulus(), collector, plan)
