...
Spikenaut is the small-supervisor layer of a wider research program, **Artificial Interoception / Neuromorphic Supervisor** ([#7](https://github.com/rmems/Spikenaut-SNN/issues/7)). This repository holds the model artifact — the shipped weight files and the Q8.8 export contract. Two caveats the name invites: the weights are not established as trained (Status, below), and there is no decision contract — nothing in this repository or its history defines what the three output rows mean.

![Spikenaut-SNN-v2 architecture: Input to Linear to LIF to Output, with the Q8.8 parameter file feeding each stage and the caveat attached to each](assets/architecture.png)

## Status — read this first

> **On "this repository".** The `.mem` artifacts, `snn_model.json` and `config.json` published here are
> byte-identical to the ones in the [GitHub repository](https://github.com/rmems/Spikenaut-SNN), which is
...
This repository ships the model itself — the parameters and the definition they were exported from.
...
config.json                        # Architecture and clock, machine readable
```
...
repository](https://github.com/rmems/Spikenaut-SNN), not here:
...
`snn_model.json` and lifts it into a [NIR](https://neuroir.org) graph. Both read exactly the
...
def load_q8_8(path, signed=False):
    """Read a .mem file of 4-digit hex words into floats."""
    out = []
    for word in Path(path).read_text().split():
        code = int(word, 16)
        if signed and code >= 0x8000:      # two's complement
            code -= 0x10000
        out.append(code / 256.0)
    return out

base = Path("dataset/merged_v2")
thresholds = load_q8_8(base / "parameters.mem")                            # 16
decays     = load_q8_8(base / "parameters_decay.mem")                      # 16
weights    = load_q8_8(base / "parameters_weights.mem")                    # 256
outputs    = load_q8_8(base / "parameters_output_weights.mem", signed=True)  # 48

W = [weights[n * 16:(n + 1) * 16] for n in range(16)]   # row n = neuron n
```
...
[GitHub repository](https://github.com/rmems/Spikenaut-SNN) carries a fuller checker,
...
component contract ([#5](https://github.com/rmems/Spikenaut-SNN/issues/5)), which deliberately
...
repository](https://github.com/rmems/Spikenaut-SNN) — there is no build for it in this download.

| Component | Role | Relationship |
|---|---|---|
| [`nir-rs`](https://crates.io/crates/nir-rs) 0.4.2 | NIR graph interchange | **Adopted**, resolved from crates.io — [#8](https://github.com/rmems/Spikenaut-SNN/issues/8) |
| [`axon-encoder`](https://crates.io/crates/axon-encoder) 0.4.0 | Telemetry → spike encoding | **Adopted**, resolved from crates.io — [#9](https://github.com/rmems/Spikenaut-SNN/issues/9) |
| [`neuromod`](https://crates.io/crates/neuromod) 0.5.2 | LIF engine, learning rules, neuromodulators | Published dependency |
...
| `SynapticDistill.jl` | Training sidecar that writes the `.mem` artifacts | **Sidecar, not a library dependency** — [#13](https://github.com/rmems/Spikenaut-SNN/issues/13) |
