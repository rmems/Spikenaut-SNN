...
- **The output layer has a JSON source and a documented decision contract.** The 48 signed values in `parameters_output_weights.mem` match per-neuron `output_weights` in `snn_model.json` (neuron-major). Distill row order is `(comfort, temp, power)`. `replay_output_row` is the software replay path for an already-scored row: argmax, lowest-index ties, fail-closed on non-finite or malformed rows, overflowing derived margin/confidence/denominator, and (Python) a non-boolean `abstain_on_tie`. `score_readout` (and `replay_tick`) refuse overflowing readout sums and (Python) non-boolean spikes. JSON `output_weights` are snapped to Q8.8 against the neuron-major `.mem` image so value/order drift cannot pass. The model-bank entry for this checkpoint names the RM-1150 output-contract identifier; RM-1150 `ALLOW/WARN/THROTTLE/PAUSE/YIELD_GPU` stays unbound. `tools/verify_q88.py` still pins the `.mem` by canonical sha256 and gold hex. [#6](https://github.com/rmems/Spikenaut-SNN/issues/6), Linear RM-1328.

### Direct Q8.8 memory provenance boundary

`load_lif_graph_from_mem_dir` constructs the same `Input → Linear → LIF → Output`
graph without opening `snn_model.json`. It accepts only the four canonical file
names above, with one uppercase four-digit hexadecimal Q8.8 word per line, and
checks UTF-8, word counts, and decay semantics before graph construction. The
returned `MemBankGraph` carries the 48 neuron-major output weights separately:
the established NIR graph exposes hidden spikes, while the three-wide Distill
readout belongs to the decision module. This explicit sidecar is intentional;
the loader never silently discards an image it validated.
...
| [`axon-encoder`](https://crates.io/crates/axon-encoder) 0.5.0 | Telemetry → spike encoding | **Declared** in `Cargo.toml`, resolved from crates.io — downstream of `kinetic-signals` — [#9](https://github.com/rmems/Spikenaut-SNN/issues/9) |
