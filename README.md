...
One clause of that diagnosis looks wrong on the mathematics, independently of provenance — **as an analytical claim, not one this repository demonstrates.** No learning rule is implemented here, so nothing in this artifact executes e-prop or OTTT; the argument runs from the published definitions of the two rules, and a reader should check it against those rather than against this code. What the repository *does* establish is the premise: `src/graph.rs` builds three edges, `Input → Linear → LIF → Output`, with no back edge and fixed thresholds. For a feedforward layer of that shape the two rules are generally taken to reduce to the same update — the same eligibility trace, the same surrogate factor, the same learning-signal modulation — diverging only under recurrence, adaptive thresholds, or a different feedback-routing scheme, none of which this model has. On that reading "identical E-prop/OTTT gradients" describes a check that **passed**, not a defect that caused anything, and whatever produced the ramp was not the two rules agreeing. What *is* verifiable here is the ramp itself and that it is not an export bug. The export path was independently cross-validated and confirmed correct ([#4](https://github.com/rmems/Spikenaut-SNN/issues/4)).
