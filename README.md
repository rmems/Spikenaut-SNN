...
| **The decay rates are placeholder values.** | `parameters_decay.mem` is byte-identical (sha256 `6db9fecb…`) to the historical `converted_parameters/spikenaut_snn_v2_decay.mem`, produced by a deleted `simple_convert.py::create_parameters` whose docstring reads *"Create sample parameters"* and whose decay line is `torch.linspace(0.8, 0.95, 16)`. They are evenly spaced defaults, not learned or converted values — and they set every LIF time constant, since `tau = -dt / ln(decay)`. The model's entire temporal behaviour rests on them. The same script's thresholds and hidden weights do **not** match the shipped files, so this applies to the decay rates alone. |
...
| Decay rates (16) | **Sample defaults — never trained or converted** | `torch.linspace(0.8, 0.95, 16)`, see below |
